...
// mayinyerlestirme foksiyonumuz 15 mayinimizi 10*10 luk matrisimizde rastgele herhangi bir konuma uygun parametrelerle dagýtmamizi saðlamaktadýr.
void mayinyerlestirme(char mayintarla[10][10]);

/* konumbelirtme fonksiyonumuz kullanýcý tarafýndan girilen satýr sütun bilgisine göre açýlan nokta için tepki verir 
yani oyunu kullanýcý bu fonksiyonda somutsal sekilde kullanýr. */
int konumbelirtme(char mayintarla[10][10]);

//tarlayazdýr fonsiyonumuz oluþturulan matris dizisini ekrana bastýrýr.
void tarlayazdir(char tarla[10][10]);

//tarlaboslukdoldur fonskiyonumuz tarlamýzýn ilk hali için dizimizdeki tüm elemanlarý '-' karakteriyle doldurarak boþluklarý niteler.
void tarlaboslukdoldur(char tarla[10][10]);

//karemayinsayiyazdir fonksiyonumuz dizimizdeki karelerin üstündeki mayin sayisini hesaplayýp ekrana yazdýrýr.
void karemayinsayiyazdir(char mayintarla[10][10]);

/*aynýnoktaengelleme fonksiyonu oyunu oynarken önceden açtýðýmýz herhangi bir noktayý bir kez daha açmamamýzý engellemektedir.
(bu fonksiyon olmadýðýnda oyunu ayný noktalara tekrar tekrar basarak kazanma olasýlýðýmýz bulunuyordu.)
...
// bu döngü tüm satýrlarda ve sütunlarda bulunan mayýn sayýlarýný  sutunmayýn ve satýrmayýn dizilerinde saklamaktadýr.
for(i=0;i<10;i++) {
	sutunmayinsayi=0;
	satirmayinsayi=0;
...
//k sayýsýný kullanmamýn sebebi rastgele atanan sayýyý uzun zaman aralýklarýyla atamamk(mayýnlarýn sürekli olarak yakýnlýðýný engellemek)
//Mayýnlar uygun parametrelerle döþenmektedir.
